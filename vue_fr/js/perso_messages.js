$(function()
{
	...
	function chosen_ajaxify(id, ajax_url)
	{
		...
		while($('#' + id + '_default_' + i).length)
		{
			$("#"+id+" option:nth-child(" + i + ")").val($('#' + id + '_default_' + i).val());
			i++;
		}
	}
	...
	chosen_ajaxify('destinataires', 'ajax.php?perso_messages&pseudo=');
	
	if($('#destinataires option').length)
		$('#new_message').modal('show');
	
	$('#new_message_message').keyup(function()
	{
		if($(this).val() != '')
		{
			...
			$('#new_message_envoyer').removeAttr('disabled');
		}
		else
		{
			$('#new_message_envoyer').attr('disabled', '');
			$('#alert_message').css('display', 'none');
		}
	});
